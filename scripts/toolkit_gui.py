...
MODEL_SAVE_PATH = getattr(shared.cmd_opts, "ckpt_dir", None) or os.path.join(models_path, "Stable-diffusion")
...
VAE_SAVE_PATH = getattr(shared.cmd_opts, "vae_dir", None) or os.path.join(models_path, "VAE")
...
if getattr(shared.cmd_opts, "ckpt_dir", None):
     LOAD_PATHS += [getattr(shared.cmd_opts, "ckpt_dir", None)]
if getattr(shared.cmd_opts, "vae_dir", None):
     LOAD_PATHS += [getattr(shared.cmd_opts, "vae_dir", None)]
...
# Default safetensors __metadata__ values shown in the UI. These can be
# overridden per-save in the textboxes.
DEFAULT_MD_AUTHOR = "Cyberdelia"
DEFAULT_MD_HOMEPAGE = "https://civitai.com/user/Cyberdelia"
DEFAULT_MD_DESCRIPTION = "Stable Diffusion model created by Cyberdelia."


def build_metadata_dict(author, homepage, description):
    """Build a safetensors-compatible __metadata__ dict, skipping empty fields."""
    md = {}
    if author and author.strip():
        md["author"] = author.strip()
    if homepage and homepage.strip():
        md["homepage"] = homepage.strip()
    if description and description.strip():
        md["description"] = description.strip()
    return md
...
class ToolkitModel():
    def __init__(self):
        ...
        # safetensors __metadata__ header fields (shown/edited in UI)
        self.md_author = ""
        self.md_homepage = ""
        self.md_description = ""
...
def do_analysis(model):
    ...
    # Safety: if the best match is a standalone *-BROKEN architecture,
    # we only recognized a single component template. In that case we do
    # NOT have enough information to classify the remaining tensors as
    # "junk". Treating everything outside the single component as junk
    # can lead to catastrophic pruning recommendations (e.g. reducing an
    # SDXL checkpoint down to a tiny VAE-only file).
    if "BROKEN" in tm.a_type:
        allowed = set((k, tensor_shape(k, model[k])) for k in model.keys())
...
def do_basic_report(details: ToolkitModel, dont_half, keep_ema):
    ...
    if "BROKEN" in d.a_type:
        report += ("**Warning:** This is a *standalone component* match (e.g. only VAE/UNet/CLIP template matched). "
                   "Prune/junk estimates may be unreliable for full checkpoints.\n\n")
    out = []
...
def do_load(source, precision):
    ...
    if source.startswith("NEW "):
        ...
        # Seed metadata fields with defaults for brand-new models
        loaded.md_author = DEFAULT_MD_AUTHOR
        loaded.md_homepage = DEFAULT_MD_HOMEPAGE
        loaded.md_description = DEFAULT_MD_DESCRIPTION
    else:
        filename = find_source(source)
        if not filename:
            error = f"Cannot find {source}!"
        else:
            model, file_metadata = load(filename)
            ...
            # Populate metadata fields: prefer existing header values,
            # fall back to Cyberdelia defaults when missing.
            loaded.md_author = file_metadata.get("author", DEFAULT_MD_AUTHOR) if file_metadata else DEFAULT_MD_AUTHOR
            loaded.md_homepage = file_metadata.get("homepage", DEFAULT_MD_HOMEPAGE) if file_metadata else DEFAULT_MD_HOMEPAGE
            loaded.md_description = file_metadata.get("description", DEFAULT_MD_DESCRIPTION) if file_metadata else DEFAULT_MD_DESCRIPTION
    if loaded:
    ...
    if loaded:
        md_updates = [
            gr.update(value=loaded.md_author),
            gr.update(value=loaded.md_homepage),
            gr.update(value=loaded.md_description),
        ]
    else:
        md_updates = [gr.update(), gr.update(), gr.update()]
    ...
    updates = reports + sources + drops + rows + names + error + md_updates
    return updates
...
def do_clear():
    ...
    md_updates = [
        gr.update(value=DEFAULT_MD_AUTHOR),
        gr.update(value=DEFAULT_MD_HOMEPAGE),
        gr.update(value=DEFAULT_MD_DESCRIPTION),
    ]
...
def do_save(save_name, precision, md_author, md_homepage, md_description):
    dont_half = "FP32" in precision
    keep_ema = False
    ...
    if model:
        user_md = build_metadata_dict(md_author, md_homepage, md_description)
        if filename.endswith(".safetensors"):
            # For safetensors, embed the user-supplied metadata in the
            # __metadata__ header (flat string→string dict).
            save(model, user_md, filename)
        else:
            # For legacy .ckpt/.pt, keep the original pytorch-lightning
            # style training metadata so loaders don't choke.
            save(model, dict(METADATA), filename)
    else:
...
def do_export(drop_arch, drop_class, drop_comp, export_name, precision, md_author, md_homepage, md_description):
    dont_half = "FP32" in precision
    error = ""
    ...
    if not loaded or not loaded.model:
        ...
        if comp:
            ...
            # Embed author/homepage/description metadata when exporting
            # components to .safetensors. For legacy .pt/.ckpt component
            # files we keep an empty metadata dict to match prior behavior.
            if filename.endswith(".safetensors"):
                user_md = build_metadata_dict(md_author, md_homepage, md_description)
                save(model, user_md, filename)
            else:
...
def on_ui_tabs():
    ...
    """
    with gr.Blocks(css=css, analytics_enabled=False) as checkpoint_toolkit:
        ...
        # Safetensors __metadata__ editor. Shown once a model is loaded.
        with gr.Accordion("Safetensors metadata (author / homepage / description)", open=False):
            with gr.Row():
                md_author_tb = gr.Textbox(label="Author", value=DEFAULT_MD_AUTHOR, interactive=True)
                md_homepage_tb = gr.Textbox(label="Homepage", value=DEFAULT_MD_HOMEPAGE, interactive=True)
            md_description_tb = gr.Textbox(label="Description", value=DEFAULT_MD_DESCRIPTION, lines=2, interactive=True)
        with gr.Tab("Basic"):
        ...
        md_fields = [md_author_tb, md_homepage_tb, md_description_tb]

        everything = reports + sources + drops + rows + names + error + md_fields
        ...
        save_button.click(
            fn=do_save,
            inputs=[save_name, prec_dropdown] + md_fields,
            outputs=everything,
        )

        export_button.click(
            fn=do_export,
            inputs=drops + [export_name, prec_dropdown] + md_fields,
            outputs=drops + [export_name] + error,
        )
...
def on_ui_settings():
    section = ('model-toolkit', "Model Toolkit Reloaded")
