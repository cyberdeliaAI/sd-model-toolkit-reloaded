...
#
# Some checkpoints (especially SDXL variants and many merged models) omit
# certain non-essential CLIP buffers such as `embeddings.position_ids`.
# This tensor is a convenience buffer that can be regenerated at runtime.
#
# The original Toolkit treated it as a *required* key for CLIP-v1-based
# components (including SDXL AUX CLIP), which caused the whole model
# architecture detection to fail and then fall back to a standalone
# VAE-*BROKEN classification.
#
# We treat these keys as OPTIONAL for component matching.

COMPONENT_OPTIONAL_KEY_SUFFIXES = {
    # CLIP-v1 components
    "CLIP-v1-SD": {"embeddings.position_ids"},
    "CLIP-v1-NAI": {"embeddings.position_ids"},
    # SDXL AUX CLIP uses the CLIP-v1 key list but with a different prefix
    "CLIP-XL-AUX-SD": {"embeddings.position_ids"},
}


def _filter_optional_component_keys(component_name: str, required: set):
    """Remove known-optional keys from a component key-set.

    `required` is a set of (key, shape) tuples.
    Matching is done by key suffix so it works for both prefixed and
    unprefixed variants.
    """
    opt_suffixes = COMPONENT_OPTIONAL_KEY_SUFFIXES.get(component_name, set())
    if not opt_suffixes:
        return required
    out = set()
    for k, z in required:
        if any(str(k).endswith(suf) for suf in opt_suffixes):
            continue
        out.add((k, z))
    return out
...
def inspect_model(model, all=False):
    ...
    for comp in COMPONENTS:
        # Treat some non-essential tensors (e.g. CLIP position_ids buffers)
        # as OPTIONAL so we don't misclassify valid models as "BROKEN".
        required_keys_unprefixed = _filter_optional_component_keys(comp, COMPONENTS[comp]["keys"])
        required_keys_prefixed = _filter_optional_component_keys(comp, get_prefixed_keys(comp))
...
def fix_model(model, fix_clip=False):
    ...
    # fix merging nonsense / missing buffers
    # Some checkpoints omit or corrupt position_ids buffers. This is not
    # critical for inference, but can confuse template-based tooling.
    broken = []
    pos_keys = [
        "cond_stage_model.transformer.text_model.embeddings.position_ids",  # SD1/SD2 CLIP
        "conditioner.embedders.0.transformer.text_model.embeddings.position_ids",  # SDXL AUX CLIP
    ]
    for i in pos_keys:
        if i in model:
            ...
            diff = correct.ne(current)
            broken = [j for j in range(77) if diff[0][j]]
            ...
            # If asked to fix, inject the buffer only when the surrounding
            # module is actually present (avoid adding SD1 keys to SDXL and vice versa).
            if fix_clip:
                if i.startswith("cond_stage_model."):
                    if any(k.startswith("cond_stage_model.transformer.text_model.embeddings.") for k in model):
                        model[i] = torch.Tensor([list(range(77))]).to(torch.int64)
                elif i.startswith("conditioner.embedders.0.transformer.text_model."):
                    if any(k.startswith("conditioner.embedders.0.transformer.text_model.embeddings.") for k in model):
                        model[i] = torch.Tensor([list(range(77))]).to(torch.int64)
...
def load(file):
    ...
    if file.endswith(".safetensors") or file.endswith(".st"):
        # Read the safetensors __metadata__ header so we can preserve /
        # display it in the UI. load_file() silently drops it.
        try:
            with safetensors.safe_open(file, framework="pt", device="cpu") as f:
                header_md = f.metadata()
                if header_md:
                    metadata = dict(header_md)
        except Exception as e:
            print(f"[model-toolkit] Could not read safetensors metadata: {e}")
    ...
    if file.endswith(".safetensors"):
        ...
        if metadata:
            ...
            for k, v in metadata.items():
                if v is None:
                    continue
                st_metadata[str(k)] = v if isinstance(v, str) else str(v)
            if not st_metadata:
                st_metadata = None
        safetensors.torch.save_file(model, file, metadata=st_metadata)
        return
    else:
        out = metadata if metadata else {}
